...
const root='D:/DevSpace/devspace', buildId='chat-goal-card-preview-20260908-03';
const candidate=join('D:/DevSpace-Goal-PoC/.poc/replan-v1/releases',buildId);
const backup='D:/DevSpace-Goal-PoC/.poc/replan-v1/evidence/deploy-chat-card-20260908-03-before';
...
try {
  ...
  if(mode==='prepare'){
    assert.equal(pointer.buildId,'chat-goal-card-preview-20260908-02');
    const previous=JSON.parse(execFileSync(process.execPath,[join(import.meta.dirname,'chat-card-v2-deploy-guard.mjs'),'verify'],{encoding:'utf8',windowsHide:true}));
