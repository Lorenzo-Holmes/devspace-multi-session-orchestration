...
const sharedStyle=await readFile(join(root,'scripts/card-system.css'),'utf8');
const injectStyle=(template)=>template.includes('<!--CARD_STYLE-->')?template.replace('<!--CARD_STYLE-->',()=>`<style>${sharedStyle}</style>`):template;
...
const template=injectStyle(await readFile(join(root,'scripts/card-lab/card.html'),'utf8'));
...
const goalTemplate=injectStyle(await readFile(join(root,'scripts/goal-card/card.html'),'utf8'));
...
const computerTemplate=injectStyle(await readFile(join(root,'scripts/computer-approval/card.html'),'utf8'));
...
const supervisorTemplate=injectStyle(await readFile(join(root,'scripts/supervisor/card.html'),'utf8'));
