...
- the bundled `computer-use` skill when Windows Computer Use is enabled, unless `~/.devspace/skills/computer-use/SKILL.md` exists
...
When `DEVSPACE_COMPUTER_USE=1` is present on Windows, `open_workspace` also
advertises the `computer-use` skill. It requires an observe → one action → observe
loop and teaches both the native `observe`/`computer` tools and the frozen-tool
compatibility path (`read("@desktop")` plus `exec_command("@computer …")`). A
user copy under `~/.devspace/skills/computer-use/SKILL.md` overrides the bundled
skill without changing the server build.
