...
function hasSkill(skillDir: string, name: string): boolean {
  return existsSync(join(skillDir, bundledSkillPath(name)));
}

export function effectiveSkillPaths(config: ServerConfig, cwd: string): string[] {
  const bundledSkills = bundledSkillsDir();
  const needsBundledSubagents = config.subagents.enabled
    && !hasSkill(config.devspaceSkillsDir, SUBAGENTS_SKILL_NAME);
  const needsBundledComputerUse = config.computerUseEnabled
    && !hasSkill(config.devspaceSkillsDir, COMPUTER_USE_SKILL_NAME);
  const defaultPathCandidates = [
    join(homedir(), ".agents", "skills"),
    resolve(cwd, ".agents", "skills"),
    config.devspaceSkillsDir,
    join(config.agentDir, "skills"),
    needsBundledSubagents || needsBundledComputerUse
      ? bundledSkills
      : undefined,
  ];
...
export function loadWorkspaceSkills(config: ServerConfig, cwd: string): LoadedSkills {
  ...
  if (!config.subagents.enabled) hiddenSkillNames.add(SUBAGENTS_SKILL_NAME);
  if (!config.computerUseEnabled) hiddenSkillNames.add(COMPUTER_USE_SKILL_NAME);
  return {
    skills: result.skills.filter((skill) => !hiddenSkillNames.has(skill.name)),
    diagnostics: result.diagnostics.filter((diagnostic) => {
      const collision = diagnostic.collision;
      return !(
        collision?.resourceType === "skill"
        && hiddenSkillNames.has(collision.name)
      );
    }),
  };
}
