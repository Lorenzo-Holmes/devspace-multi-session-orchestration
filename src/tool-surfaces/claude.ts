...
const CLAUDE_INSTRUCTIONS = `Use ${toolNames.read} for direct file reads, ${toolNames.listDirectory} for directory inspection, ${toolNames.fileInfo} for path metadata, ${toolNames.batchReadFiles} for bounded multi-file reads, ${toolNames.searchFiles} for filename/content search, and ${toolNames.querySqlite} for read-only SQLite queries. Use ${toolNames.edit} for targeted modifications and ${toolNames.write} only for new files or complete rewrites. Reserve ${toolNames.shell} for tests, builds, package scripts, long-running processes, and inspection not covered by the native read-only tools. Shell commands run with the local user's authority and are not sandboxed; workspace validation only selects their initial working directory. Follow instructions returned by ${toolNames.openWorkspace}; read applicable instruction and skill files before working in their scope.`;
...
const CLAUDE_SHELL_DESCRIPTION = `Run a shell command with the local user's authority. Commands are not sandboxed; workspace validation only selects the initial working directory. Prefer native read-only filesystem/search/SQLite tools for routine inspection; use Bash for tests, builds, package scripts, long-running processes, and inspection those tools cannot express.`;
