...
test("a nested workspace never snapshots or reads its parent repository", async (t) => {
  const parent = await committedRepository(t);
  const root = join(parent, "nested-project");
  await mkdir(root);
  await writeFile(join(parent, "outside-workspace.txt"), "must not enter review objects\n");
  const index = await readFile(join(parent, ".git", "index"));
  const refs = await gitOutput(parent, ["show-ref"]);
  const objects = await gitOutput(parent, ["count-objects", "-v"]);
  const head = await gitOutput(parent, ["rev-parse", "HEAD"]);
  const manager = createReviewCheckpointManager();
  const availability = await manager.initializeWorkspace({ workspaceId: "ws_nested", root });
  assert.equal(availability.available, false);
  if (!availability.available) assert.match(availability.reason, /parent repositories are outside/);
  await assert.rejects(() => manager.reviewChanges({ workspaceId: "ws_nested", root }), /outside its scope/);
  await assert.rejects(() => manager.reviewByRef({ workspaceId: "ws_nested", root, reviewRef: head }), /outside its scope/);
  await assert.rejects(() => readReviewRef(root, head), /outside its scope/);
  assert.deepEqual(await readFile(join(parent, ".git", "index")), index);
  assert.equal(await gitOutput(parent, ["show-ref"]), refs);
  assert.equal(await gitOutput(parent, ["count-objects", "-v"]), objects);
});

test("a linked worktree rooted at the workspace remains reviewable", async (t) => {
  const parent = await committedRepository(t);
  const temporary = await mkdtemp(join(tmpdir(), "devspace-review-linked-test-"));
  t.after(() => rm(temporary, { recursive: true, force: true }));
  const root = join(temporary, "worktree");
  await git(parent, ["worktree", "add", "--detach", root, "HEAD"]);
  const manager = createReviewCheckpointManager();
  assert.deepEqual(await manager.initializeWorkspace({ workspaceId: "ws_linked", root }), { available: true });
  await writeFile(join(root, "inside.txt"), "inside linked workspace\n");
  const review = await manager.reviewChanges({ workspaceId: "ws_linked", root });
  assert.deepEqual(review.files.map(file => file.path), ["inside.txt"]);
});
