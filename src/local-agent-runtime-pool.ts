...
export interface LocalAgentRuntimeHold {
  readonly runtime: LocalAgentRuntime;
  bindSession(providerSessionId: string): Promise<void>;
  /** Caller must pause/stop its native Goal before releasing the hold. */
  release(): void;
}
...
export class LocalAgentRuntimePool {
  ...
  /** A named owner holds native Goal resources across turn-completed events. */
  async hold(
    driver: LocalAgentDriver,
    context: LocalAgentRuntimeContext,
  ): Promise<BetterResult<LocalAgentRuntimeHold, AgentProviderError>> {
    if (this.closing) return Result.err(poolClosedError(driver, context));
    const acquired = await this.acquire(driver, context);
    if (acquired.isErr()) return acquired;
    const entry = acquired.value;
    const runtime = entry.runtime;
    if (!runtime || !runtime.isAlive() || entry.closing || this.closing) {
    ...
    return Result.ok({
      runtime,
      bindSession: async (providerSessionId) => {
        if (!providerSessionId) throw new Error("A runtime hold requires a non-empty session id.");
        while (true) {
          if (released || entry.closing || this.closing) throw poolClosedError(driver, context);
          if (sessionIds.has(providerSessionId)) return;
          const existing = entry.sessions.get(providerSessionId);
          if (existing?.releasePromise) { await existing.releasePromise; continue; }
      ...
      release: () => {
        ...
        for (const id of sessionIds) {
          ...
          if (session) { session.activeRuns -= 1; session.lastUsedAt = this.now(); }
        }
        entry.activeRuns -= 1;
        ...
        if (entry.activeRuns === 0) {
          ...
          if (!entry.closing) this.scheduleIdleClose(entry);
        }
      },
    });
  }
