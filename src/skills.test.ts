...
try {
  ...
  assert.equal(loaded.skills.some((skill) => skill.name === "computer-use"), false);
  ...
  const computerUseConfig = loadConfig({
    ...writeTestDevspaceConfig(configDir, {
      server: { port: 1 },
      workspaces: { allowedRoots: [projectRoot] },
      skills: { agentDir },
    }),
  ...
  const computerUseSkill = loadWorkspaceSkills(computerUseConfig, projectRoot).skills.find(
    (skill) => skill.name === "computer-use",
  );
  assert.ok(computerUseSkill);
  assert.match(computerUseSkill.description, /Windows desktop apps through DevSpace/i);

  await mkdir(join(devspaceSkills, "computer-use"), { recursive: true });
  await writeFile(
    join(devspaceSkills, "computer-use", "SKILL.md"),
    [
      "---",
      "name: computer-use",
      "description: Local DevSpace Computer Use override.",
      "---",
      "",
      "# Local Computer Use",
    ].join("\n"),
  );
  const localComputerUseSkill = loadWorkspaceSkills(computerUseConfig, projectRoot).skills.find(
    (skill) => skill.name === "computer-use",
  );
  assert.ok(localComputerUseSkill);
  assert.equal(localComputerUseSkill.description, "Local DevSpace Computer Use override.");
