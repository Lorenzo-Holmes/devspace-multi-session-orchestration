...
export type { WorkspaceAccessMode } from "./workspace-store.js";
...
export class WorkspaceRegistry {
  ...
  async openWorkspace(
    ...
    const allowedRoots = openOptions.allowedRoots ?? this.config.allowedRoots;
    const accessMode = openOptions.accessMode ?? "modify";
    const accessGrantId = openOptions.accessGrantId;
    if (!conversationScopeId || !this.store) {
      return this.openNewWorkspace(workspaceInput, allowedRoots, accessMode, accessGrantId);
    }

    const projectKey = await this.conversationProjectKey(workspaceInput, allowedRoots);
  ...
  private async openNewWorkspace(
    options: OpenWorkspaceInput,
    allowedRoots: string[],
    accessMode: WorkspaceAccessMode,
    ...
    return this.openCheckoutWorkspace(options.path, allowedRoots, accessMode, accessGrantId);
  }
  ...
  private async openConversationCheckout(
    ...
    if (binding) {
      const reusableWorkspace = await this.findReusableCheckoutWorkspace(binding, allowedRoots);

      if (reusableWorkspace) {
        reusableWorkspace.accessMode = accessMode;
        reusableWorkspace.accessGrantId = accessGrantId;
    ...
    const context = await this.openCheckoutWorkspace(
      input.path,
      allowedRoots,
      accessMode,
      accessGrantId,
  ...
  private async conversationProjectKey(
    input: OpenWorkspaceInput,
    allowedRoots: string[],
  ): Promise<string> {
    const path = assertAllowedPath(input.path, allowedRoots);
  ...
  getWorkspace(workspaceId: string, options: { touch?: boolean } = {}): Workspace {
    const workspace = this.workspaces.get(workspaceId);
    if (workspace) {
      if (options.touch !== false) this.store?.touchSession(workspaceId);
      return workspace;
    }
    ...
    const allowedRoots = this.restoreAllowedRoots?.(session) ?? this.config.allowedRoots;
  ...
  private async openCheckoutWorkspace(
    ...
    const root = assertAllowedPath(path, allowedRoots);
  ...
  private async createWorkspaceContext(input: {
    ...
    this.store?.createSession({
      ...
      accessGrantId: workspace.accessGrantId,
    });
  ...
  private assertWorkspaceRootAllowed(
    ...
    allowedRoots: string[] = this.config.allowedRoots,
  ): string {
    if (mode === "worktree") {
      if (!sourceRoot) {
        throw new Error(`Stored worktree workspace is missing sourceRoot: ${root}`);
      }
      assertAllowedPath(sourceRoot, allowedRoots);
      return assertAllowedPath(root, [this.config.worktreeRoot]);
    }

    return assertAllowedPath(root, allowedRoots);
  }
