...
export const devspaceConfigSchema = z.object({
  ...
  goals: z.object({
    enabled: z.boolean().default(false),
    shrimpEntryPoint: z.string().default(""),
    dataRoot: z.string().default(""),
    codexCommand: z.string().optional(),
  }).strict().refine(value => !value.enabled || (!!value.shrimpEntryPoint.trim() && !!value.dataRoot.trim()),
    "Enabled Goals require shrimpEntryPoint and dataRoot.").prefault({}),
  chatGoals: z.object({
    enabled: z.boolean().default(false),
    shrimpEntryPoint: z.string().default(""),
    dataRoot: z.string().default(""),
  }).strict().refine(value => !value.enabled || (!!value.shrimpEntryPoint.trim() && !!value.dataRoot.trim()),
    "Enabled Chat Goals require shrimpEntryPoint and dataRoot.").prefault({}),
  diagnostics: z.object({
    chatCard: z.boolean().default(false),
  }).strict().prefault({}),
