...
export interface LocalAgentDaemonOptions {
  ...
  onClosed?: () => void;
  goals?: GoalApi & { readonly activeCount:number; maintain():Promise<void>; close():Promise<void> };
}
...
export class LocalAgentDaemon {
  ...
  private readonly goals?: LocalAgentDaemonOptions['goals'];
  ...
  async close(): Promise<void> {
    ...
    this.closePromise = (async () => {
      ...
      const [serverResult, managerResult] = await Promise.allSettled([
        ...
        withTimeout((async()=>{await this.goals?.close();await this.manager.close();})(), this.shutdownTimeoutMs, "daemon manager shutdown"),
      ]);
  ...
  private async dispatch(request: LocalAgentDaemonRequest): Promise<unknown> {
    ...
    switch (request.method) {
      ...
      case "goal.request":
        return this.goals ? this.goals.goal(request.params) : {ok:false,error:{code:'GOAL_CAPABILITY_UNAVAILABLE',message:'Managed Goals are not enabled in this daemon build/configuration.',retryable:false}};
      case "daemon.status":
        return this.status();
      case "daemon.stop":
        this.stopping = true;
  ...
  private async maintainIdle(): Promise<void> {
    await this.goals?.maintain();
    await this.manager.evictIdle(this.now());
    if (this.stopping || this.manager.activeTurnCount > 0 || this.manager.runtimeCount > 0 || this.sockets.size > 0 || (this.goals?.activeCount ?? 0) > 0) {
