...
export function decodeToolResult(result: CallToolResult): DecodedToolResult {
  ...
  if (structured) {
    const requestId = stringField(structured.requestId);
    const requestPath = stringField(structured.path);
    const requestedAccess = accessMode(structured.requestedAccess ?? structured.access);
    const accessStatus = accessStatusField(structured.status);
    if (requestId && requestPath && requestedAccess && accessStatus) {
      ...
      return {
        ...
        card: {
          ...structuredCard,
          ...metaCard,
          tool: accessStatus === "pending" || accessStatus === "already_allowed"
  ...
  if (
    metaCard?.tool === "request_workspace_access"
    && metaCard.requestId
    && metaCard.path
    && metaCard.requestedAccess
    && metaCard.status
  ) {
    return { kind: "card", card: metaCard as ToolResultCard };
  }
  if (
    metaCard?.tool === "approve_workspace_access"
    && metaCard.path
    && metaCard.requestedAccess
    && metaCard.status
  ) {
...
function cardFields(record: Record<string, unknown> | undefined): Partial<ToolResultCard> | undefined {
  ...
  return definedFields({
    ...
    accessMode: accessMode(record.accessMode),
    requestId: stringField(record.requestId),
    approvalToken: stringField(record.approvalToken),
    requestedAccess: accessMode(record.requestedAccess ?? record.access),
    status: accessStatusField(record.status),
    risk: accessRiskField(record.risk),
    temporaryScopeAvailable: booleanField(record.temporaryScopeAvailable),
    reason: stringField(record.reason),
    createdAt: stringField(record.createdAt),
    expiresAt: stringField(record.expiresAt),
    decision: accessDecisionField(record.decision),
    message: stringField(record.message ?? record.result),
  });
}
...
function toolName(value: unknown): ToolResultCard["tool"] | undefined {
  return value === "open_workspace"
    || value === "show_changes"
    || value === "request_workspace_access"
    || value === "approve_workspace_access"
    ? value
    : undefined;
}

function accessMode(value: unknown): ToolResultCard["requestedAccess"] | undefined {
  return value === "read" || value === "modify" ? value : undefined;
}

function accessStatusField(value: unknown): ToolResultCard["status"] | undefined {
  return value === "pending"
    || value === "already_allowed"
    || value === "approved"
    || value === "denied"
    ? value
    : undefined;
...
function accessRiskField(value: unknown): ToolResultCard["risk"] | undefined {
  return value === "standard" || value === "high" ? value : undefined;
}

function accessDecisionField(value: unknown): ToolResultCard["decision"] | undefined {
  return value === "once"
    || value === "session"
    || value === "permanent"
    || value === "deny"
