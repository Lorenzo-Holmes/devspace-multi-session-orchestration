...
test("workspace access cards combine public fields with the app-only approval token", () => {
  const decoded = decodeToolResult({
    content: [],
    structuredContent: {
      result: "Approval required.",
      requestId: "access_123",
      path: "D:\\Project",
      requestedAccess: "modify",
      status: "pending",
      risk: "standard",
      temporaryScopeAvailable: true,
    ...
    _meta: {
      card: {
        ...
        requestedAccess: "modify",
        status: "pending",
        approvalToken: "private-token-value",
      },
    },
  });
  ...
  assert.equal(decoded.kind, "card");
  if (decoded.kind !== "card") return;
  assert.equal(decoded.card.tool, "request_workspace_access");
  assert.equal(decoded.card.approvalToken, "private-token-value");
  assert.equal(decoded.card.temporaryScopeAvailable, true);
});

test("workspace access decisions render as completed approval cards", () => {
  const decoded = decodeToolResult({
    content: [],
    structuredContent: {
      result: "Approved.",
  ...
  assert.equal(decoded.kind, "card");
  if (decoded.kind !== "card") return;
  assert.equal(decoded.card.tool, "approve_workspace_access");
  assert.equal(decoded.card.status, "approved");
  assert.equal(decoded.card.decision, "session");
});
