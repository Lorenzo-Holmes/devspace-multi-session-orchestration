...
let accessDecisionPending = false;
let accessDecisionError: string | null = null;
...
async function boot(): Promise<void> {
  ...
  app = new App(
    { name: "devspace-tool-cards", version: "0.5.0" },
    {},
  );
...
function render(): void {
  ...
  if (
    card.tool === "request_workspace_access"
    || card.tool === "approve_workspace_access"
  ) {
    renderAccessCard(card);
    return;
  }
...
function renderAccessCard(accessCard: ToolResultCard): void {
  unmountPayload();
  const main = element("main", { className: "shell" });
  const section = element("section", { className: "tool-card permission access-card" });
  const header = element("div", { className: "access-header" });
  const icon = element("span", { className: "tool-icon", ariaHidden: "true" });
  icon.append(renderIcon(toolIcons.permission));
  const heading = element("span", { className: "tool-main" });
  heading.append(
    element("span", {
      className: "tool-title",
      text: accessCard.status === "approved"
        ? "目录权限已授予"
        : accessCard.status === "denied"
          ? "目录权限已拒绝"
          : accessCard.status === "already_allowed"
            ? "目录权限已存在"
            : "DevSpace 请求目录权限",
    }),
    element("span", {
      className: "tool-label",
      text: accessCard.path ?? "未知目录",
      title: accessCard.path,
    }),
  );
  header.append(icon, heading);

  const body = element("div", { className: "access-body" });
  const details = element("div", { className: "access-details" });
  details.append(
    accessDetail("权限", accessCard.requestedAccess === "read" ? "只读" : "修改（读、写、创建、删除、重命名和运行命令）"),
    accessDetail("原因", accessCard.reason ?? "当前任务需要访问这个目录"),
    accessDetail("范围", "仅限上方显示的目录及其子目录"),
  );
  body.append(details);

  if (accessCard.risk === "high" && accessCard.status === "pending") {
    const warning = element("label", { className: "access-risk" });
    const checkbox = document.createElement("input");
    checkbox.type = "checkbox";
    checkbox.checked = highRiskConfirmed;
    checkbox.disabled = accessDecisionPending;
    checkbox.addEventListener("change", () => {
      highRiskConfirmed = checkbox.checked;
      render();
    });
    warning.append(
      checkbox,
      element("span", {
        text: "这是磁盘根目录或系统敏感目录。我确认授权范围和风险。",
      }),
    );
    body.append(warning);
  }

  const statusMessage = accessDecisionError
    ?? accessCard.message
    ?? (accessCard.status === "already_allowed"
      ? "无需重复授权，可以继续打开该工作区。"
      : accessCard.status === "approved"
        ? "权限已生效，无需重启 DevSpace。"
        : accessCard.status === "denied"
          ? "未修改任何目录配置。"
          : undefined);
  if (statusMessage) {
    body.append(element("div", {
      className: `access-status${accessDecisionError ? " error" : ""}`,
      text: statusMessage,
    }));
  }

  if (accessCard.status === "pending") {
    const actions = element("div", { className: "access-actions" });
    const highRiskBlocked = accessCard.risk === "high" && !highRiskConfirmed;
    actions.append(
      accessDecisionButton("允许一次", "once", accessCard, highRiskBlocked),
      accessDecisionButton(
        "本次对话允许",
        "session",
        accessCard,
        highRiskBlocked || accessCard.temporaryScopeAvailable === false,
        accessCard.temporaryScopeAvailable === false ? "neutral" : "primary",
      ),
      accessDecisionButton(
        "永久允许",
        "permanent",
        accessCard,
        highRiskBlocked,
        accessCard.temporaryScopeAvailable === false ? "primary" : "neutral",
      ),
      accessDecisionButton("拒绝", "deny", accessCard, false, "danger"),
    );
    body.append(actions);
    if (accessCard.temporaryScopeAvailable === false) {
      body.append(element("div", {
        className: "access-footnote",
        text: "当前客户端没有提供对话标识，因此只能永久允许或拒绝。",
      }));
    }
  }
...
function accessDetail(label: string, value: string): HTMLElement {
  const row = element("div", { className: "access-detail" });
  row.append(
    element("span", { className: "access-key", text: label }),
    element("span", { className: "access-value", text: value, title: value }),
  );
  return row;
}
...
function accessDecisionButton(
  label: string,
  decision: "once" | "session" | "permanent" | "deny",
  accessCard: ToolResultCard,
  blocked: boolean,
  tone: "primary" | "danger" | "neutral" = "neutral",
): HTMLButtonElement {
  const button = element("button", {
    className: `access-action ${tone}`,
    type: "button",
    text: accessDecisionPending ? "处理中…" : label,
    disabled: accessDecisionPending || blocked,
  });
  button.addEventListener("click", () => {
    void decideWorkspaceAccess(accessCard, decision);
  });
  return button;
}

async function decideWorkspaceAccess(
  accessCard: ToolResultCard,
  decision: "once" | "session" | "permanent" | "deny",
): Promise<void> {
  if (!app || !app.getHostCapabilities()?.serverTools) {
    accessDecisionError = "当前网页客户端无法调用 DevSpace 授权工具。";
    render();
    return;
  }
  if (!accessCard.requestId || !accessCard.approvalToken) {
    accessDecisionError = "授权卡片缺少一次性凭据，请重新发起目录权限请求。";
    render();
    return;
  }

  accessDecisionPending = true;
  accessDecisionError = null;
  render();
  const approvalArguments = {
    requestId: accessCard.requestId,
    approvalToken: accessCard.approvalToken,
    decision,
    confirmHighRisk: accessCard.risk === "high" ? highRiskConfirmed : undefined,
  };
  const bridgeLabel = "MCP Apps tools/call";
  try {
    ...
    const decoded = decodeToolResult(result);
    if (decoded.kind !== "card") {
      throw new Error("DevSpace 返回了无法识别的授权结果。请重新发起请求。");
    }
    setCard(decoded.card);
    await notifyConversationOfAccessDecision(decoded.card);
  } catch (decisionError) {
    accessDecisionPending = false;
    const message = unknownErrorText(decisionError);
    accessDecisionError = `${message}（调用通道：${bridgeLabel}）`;
    render();
  }
}

async function notifyConversationOfAccessDecision(resultCard: ToolResultCard): Promise<void> {
  if (!app || !resultCard.path || !resultCard.requestedAccess || !resultCard.status) return;
  const message = resultCard.status === "approved"
    ? `我已在 DevSpace 权限卡片中批准 ${resultCard.path} 的 ${resultCard.requestedAccess} 权限（${accessDecisionLabel(resultCard.decision)}）。请现在重新调用 open_workspace，并继续刚才的任务。`
    : `我已拒绝 DevSpace 对 ${resultCard.path} 的权限请求。除非我重新提出，否则不要继续访问该目录。`;
  try {
    ...
    // The decision is already applied server-side. The visible card remains the
    // source of truth if a host does not support app-authored chat messages.
  }
}

function accessDecisionLabel(
  decision: ToolResultCard["decision"],
): string {
...
function callToolResultText(result: CallToolResult): string {
  const text = result.content
    ...
    .join("\n")
    .trim();
  return text || "DevSpace 未能应用这次授权决定。";
}
...
function unknownErrorText(error: unknown): string {
  ...
  if (error && typeof error === "object") {
    try {
      const fields = Object.getOwnPropertyNames(error).map((key) => [
        key,
      ...
      // Fall through to the host's string representation.
    }
  }
  return String(error);
}
