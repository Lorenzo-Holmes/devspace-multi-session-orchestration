...
async function testDatabaseConfiguration(stateDir: string): Promise<void> {
  ...
  try {
    ...
    assert.deepEqual(migrations, [
      ...
      { version: 7, name: "workspace-access-approvals" },
      { version: 8, name: "managed-goal-bindings" },
      { version: 9, name: "managed-goal-control-observations" },
      { version: 10, name: "chat-goal-controller-metadata" },
      { version: 11, name: "multi-session-orchestration-v1" },
      { version: 12, name: "multi-session-coordinator-v2" },
      { version: 13, name: "coordinator-worktree-bindings-v2" },
...
async function testProviderRestartRotationAndRevocation(stateDir: string): Promise<void> {
  ...
  try {
    ...
    assert.deepEqual(verified.scopes, ["devspace", "offline_access"]);
    ...
    assert.equal(refreshed.scope, "devspace offline_access");

    await assert.rejects(
      secondProvider.exchangeRefreshToken(client, issued.refresh_token, ["devspace", "offline_access"], mcpUrl),
      InvalidGrantError,
    );
    ...
    await assert.rejects(
      secondProvider.exchangeRefreshToken(client, refreshed.refresh_token, ["devspace", "offline_access"], mcpUrl),
      InvalidGrantError,
    );
  } finally {
