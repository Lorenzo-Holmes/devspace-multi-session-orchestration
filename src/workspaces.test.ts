...
test("persisted temporary-grant workspaces restore with only their validated grant root", async (t) => {
  const context = await fixture(t);
  const stateDir = join(context.root, ".temporary-grant-state");
  const firstStore = new SqliteWorkspaceStore(stateDir);
  const session = firstStore.createSession({
    id: "ws_temporarygrant",
    root: context.outsideRoot,
    mode: "checkout",
    accessMode: "read",
    accessGrantId: "grant_test",
  });
  firstStore.close();

  const secondStore = new SqliteWorkspaceStore(stateDir);
  try {
    const withoutGrantResolver = new WorkspaceRegistry(context.config, secondStore);
    assert.throws(() => withoutGrantResolver.getWorkspace(session.id), /outside allowed roots/);

    const restoredRegistry = new WorkspaceRegistry(
      context.config,
      secondStore,
      (stored) => stored.accessGrantId === "grant_test"
        ? [...context.config.allowedRoots, context.outsideRoot]
        : context.config.allowedRoots,
    );
    const restored = restoredRegistry.getWorkspace(session.id);
    assert.equal(restored.root, context.outsideRoot);
    assert.equal(restored.accessGrantId, "grant_test");
  } finally {
    secondStore.close();
  }
});
