...
assert.deepEqual(store.list({ workspaceRoot: join(root, "other") }), []);
  ...
  legacy.exec(`
    -- The fixture records workspace migration 1 as applied below. Include its
    -- table so later additive permission migrations have a valid legacy base.
    create table workspace_sessions (id text primary key);
