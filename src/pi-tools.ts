...
export async function readFileTool(input: ReadToolInput, context: ToolContext): Promise<ToolResponse> {
  const path = await resolveRealAllowedPath(
    input.path,
    context.cwd,
...
export async function findFilesTool(
  input: FindToolInput,
  context: ToolContext,
): Promise<ToolResponse<FindToolDetails>> {
  const path = await resolveRealAllowedPath(input.path ?? ".", context.cwd, [context.root]);
...
export async function grepFilesTool(
  input: GrepToolInput,
  context: ToolContext,
): Promise<ToolResponse<GrepToolDetails>> {
  const path = await resolveRealAllowedPath(input.path ?? ".", context.cwd, [context.root]);
...
async function resolveRealAllowedPath(
  inputPath: string,
  cwd: string,
  allowedRoots: string[],
): Promise<string> {
  const lexicalPath = resolveAllowedPath(inputPath, cwd, allowedRoots);
