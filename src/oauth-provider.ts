...
export function oauthSupportedScopes(scopes: string[]): string[] {
  return Array.from(new Set([...scopes, OAUTH_OFFLINE_ACCESS_SCOPE]));
}
...
export class SingleUserOAuthProvider implements OAuthServerProvider {
  ...
  async authorize(
    ...
    if (!requestedScopesAllowed(params.scopes ?? [], oauthSupportedScopes(this.config.scopes))) {
  ...
  async verifyAccessToken(token: string): Promise<AuthInfo> {
    ...
    return {
      ...
      resource: record.resource ? new URL(record.resource) : undefined,
      // This server has one authenticated owner, not one owner per OAuth client or chat.
      extra: { devspaceOwnerRef: "single-user" },
    };
  }
