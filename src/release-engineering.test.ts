...
test("release integrity, rollback fault and benchmark-generator contracts", { timeout: 60_000 }, () => {
  const env = { ...process.env };
  delete env.NODE_TEST_CONTEXT;
  const result = spawnSync(process.execPath, ["--test", "--test-reporter=tap", "scripts/release-contract.test.mjs", "benchmarks/orchestration/fixtures.test.mjs"], {
