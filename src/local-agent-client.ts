...
import type { GoalRequest, GoalReply } from "./goal-contracts.js";
...
export class LocalAgentClient {
  async goal(input: GoalRequest): Promise<GoalReply> {
    const result = await this.request("goal.request", input);
    if (result.isErr()) return {ok:false,error:{code:result.error.code,message:result.error.message,retryable:false}};
    const reply = result.value as GoalReply;
    if(!reply || typeof reply.ok !== 'boolean')throw new Error('Invalid managed Goal daemon response.');
    return reply;
  }
