...
import type { ManagedGoalConfig } from "./goal-contracts.js";
import type { ChatGoalConfig } from "./chat-goal-contracts.js";
...
export interface ServerConfig {
  ...
  goals?: ManagedGoalConfig;
  chatGoals?: ChatGoalConfig;
  chatCardProbeEnabled?: boolean;
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): ServerConfig {
  ...
  return {
    ...
    goals: stored.goals.enabled ? {
      ...stored.goals,
      shrimpEntryPoint: normalizePath(stored.goals.shrimpEntryPoint),
      dataRoot: normalizePath(stored.goals.dataRoot),
    } : undefined,
    chatGoals: stored.chatGoals.enabled ? {
      ...stored.chatGoals,
      shrimpEntryPoint: normalizePath(stored.chatGoals.shrimpEntryPoint),
      dataRoot: normalizePath(stored.chatGoals.dataRoot),
    } : undefined,
    chatCardProbeEnabled: stored.diagnostics.chatCard,
    computerUseEnabled: env.DEVSPACE_COMPUTER_USE === "1",
